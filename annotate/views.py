...
#Returns the number of annotated sentence pairs for each languge for the current user
def stats(user):
	languages = ["de","en","fi","fr","ru","sv"]
	ret = []

	for l in languages:
		ret.append(len(Annotation.objects.filter(name = user.username, sentencepair__lang = l)))
...
#Find next pair to be annotated
def nextPair(user):
...
def annotateLogic(pair, c, username, language):
...
def annotate(request, pair_id):
	...
	user = request.user
	username = user.username
	language = user.annotator.lang
	annotateLogic(pair, c, username, language)
	return HttpResponseRedirect("/continue")
